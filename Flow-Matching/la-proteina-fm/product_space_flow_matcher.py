...
class ProductSpaceFlowMatcher(L.LightningModule):
    ...
    def full_simulation(
        ...
        with torch.no_grad():
            ...
            trajectory.append({
                'step': 0,
                'bb_ca': x["bb_ca"].clone().detach().cpu(),
                'local_latents': x["local_latents"].clone().detach().cpu(),
                'noise_level': 1.0  # 初始是纯噪声
            })
            ...
            for step in range(nsteps):
                ...
                trajectory.append({
                    ...
                    'bb_ca': x["bb_ca"].clone().detach().cpu(),
                    'local_latents': x["local_latents"].clone().detach().cpu(),
                    'noise_level': 1.0 - (step + 1) / nsteps  # 噪声逐渐减少
                })
            ...
            if save_trajectory:
                torch.save(trajectory, f'flow_trajectory_nsteps_{nsteps}.pt')
